...
const MedicineRequests = () => {
    ...
    <div className={styles.requestCard}>
      ...
      <div className={styles.requestSummary}>
        <div className={styles.summaryItem}>
          ...
          <span className={styles.summaryValue}>{request.pharmacyId.name}</span>
        </div>
        {request.orderType === 'medicine_search' && request.items && request.items.length > 0 && (
          <>
            <div className={styles.summaryItem}>
              <span className={styles.summaryLabel}>Items:</span>
              <span className={styles.summaryValue}>{request.items.length}</span>
            </div>
            <div className={styles.summaryItem}>
              <span className={styles.summaryLabel}>Total:</span>
              <span className={styles.summaryValue}>₹{request.payment?.amount?.total ?? 0}</span>
            </div>
          </>
        )}
        {request.orderType === 'prescription_upload' && request.prescription?.url && (
          <div className={styles.summaryItem}>
            <span className={styles.summaryLabel}>Prescription:</span>
            <span className={styles.summaryValue}>
              {request.prescription.url.endsWith('.pdf') ? 'PDF' : 'Image'} Uploaded
            </span>
          </div>
        )}
        {request.orderType === 'call_required' && (
          <div className={styles.summaryItem}>
            ...
            <span className={styles.summaryValue}>{request.contactNumber}</span>
          </div>
        )}
      </div>
          ...
          <div className={styles.timelineItem}>
            ...
            <div className={styles.timelineContent}>
              ...
              <div className={styles.timelineNote}>
                {request.statusHistory[request.statusHistory.length - 1].note || 'Status updated'}
              </div>
              <div className={styles.timelineDate}>
                {formatDate(request.statusHistory[request.statusHistory.length - 1].timestamp)}
              </div>
            </div>
          </div>
        )}
  ...
  const progressDialog = (
    <Dialog
      ...
      header={selectedRequest ? `Order #${selectedRequest.orderId} Details` : 'Order Details'}
    >
        ...
        {selectedRequest ? (
          <>
            ...
            {(selectedRequest.orderType === 'prescription_upload' || 
              (selectedRequest.orderType === 'call_required' && selectedRequest.prescription?.url)) && 
              selectedRequest.prescription?.url && (
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontWeight: 500, marginBottom: 6 }}>Prescription:</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  {(() => {
                    const url = selectedRequest.prescription.url;
                    if (!url) return <span>No file</span>;
                    if (url.endsWith('.pdf')) {
                      return <i className="pi pi-file-pdf" style={{ fontSize: 32, color: '#e57373' }}></i>;
                    } else {
                      return <img src={url} alt="Prescription" style={{ width: 48, height: 48, objectFit: 'cover', borderRadius: 4, border: '1px solid #e3e8ee' }} />;
                    }
                  })()}
                  <Button
            ...
            <div className={styles.orderDetailsSection}>
              ...
              <div className={styles.detailsTable}>
                <div className={styles.detailsRow}>
                  ...
                  <span className={styles.detailsValue}>#{selectedRequest.orderId}</span>
                </div>
                <div className={styles.detailsRow}>
                  <span className={styles.detailsLabel}>Order Date:</span>
                  <span className={styles.detailsValue}>{formatDate(selectedRequest.createdAt)}</span>
                </div>
                <div className={styles.detailsRow}>
                ...
                <div className={styles.detailsRow}>
                  ...
                  <span className={styles.detailsValue}>{selectedRequest.pharmacyId.name}</span>
                </div>
                <div className={styles.detailsRow}>
                  <span className={styles.detailsLabel}>Hospital/Clinic:</span>
                  <span className={styles.detailsValue}>{selectedRequest.pharmacyId.hospital}</span>
                </div>
              </div>
            </div>

            {/* Medicine Search Order Details */}
            {selectedRequest.orderType === 'medicine_search' && selectedRequest.items && selectedRequest.items.length > 0 && (
              <div className={styles.orderDetailsSection}>
                <h3>Medicines</h3>
                <div className={styles.medicinesTable}>
                  ...
                  {selectedRequest.items.map((item, index) => (
                    <div key={index} className={styles.tableRow}>
                      ...
                      <span className={styles.medicineQuantity}>{item.quantity}</span>
                      <span className={styles.medicinePrice}>₹{item.price}</span>
                      <span className={styles.medicineTotal}>₹{(item.quantity * item.price).toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {(selectedRequest.orderType === 'medicine_search' || 
              (selectedRequest.orderType === 'prescription_upload' && selectedRequest.payment?.amount)) && (
              <div className={styles.orderDetailsSection}>
                <h3>Payment Information</h3>
                <div className={styles.detailsTable}>
                  <div className={styles.detailsRow}>
                    <span className={styles.detailsLabel}>Payment Method:</span>
                    <span className={styles.detailsValue}>{selectedRequest.payment.method}</span>
                  </div>
                  <div className={styles.detailsRow}>
                    <span className={styles.detailsLabel}>Payment Status:</span>
                    <Tag
                      value={selectedRequest.payment.status}
                      severity={selectedRequest.payment.status === 'completed' ? 'success' : 'warning'}
                      className={styles.requestStatus}
                    />
                  </div>
                  {selectedRequest.payment?.amount && (
                    <>
                      <div className={styles.detailsRow}>
                        ...
                        <span className={styles.detailsValue}>₹{selectedRequest.payment.amount.subtotal ?? 0}</span>
                      </div>
                      <div className={styles.detailsRow}>
                        <span className={styles.detailsLabel}>Delivery Charge:</span>
                        <span className={styles.detailsValue}>₹{selectedRequest.payment.amount.deliveryCharge ?? 0}</span>
                      </div>
                      <div className={styles.detailsRow}>
                        <span className={styles.detailsLabel}>Tax:</span>
                        <span className={styles.detailsValue}>₹{selectedRequest.payment.amount.tax ?? 0}</span>
                      </div>
                      <div className={styles.detailsRow}>
                        <span className={styles.detailsLabel}>Total Amount:</span>
                        <span className={styles.detailsValue}>₹{selectedRequest.payment.amount.total ?? 0}</span>
                      </div>
                    </>
                  )}
                </div>
            ...
            <div className={styles.orderDetailsSection}>
              ...
              <div className={styles.detailsTable}>
                <div className={styles.detailsRow}>
                  ...
                  <span className={styles.detailsValue}>{selectedRequest.contactNumber}</span>
                </div>
                <div className={styles.detailsRow}>
                  <span className={styles.detailsLabel}>Address:</span>
                  <span className={styles.detailsValue}>
                    {selectedRequest.deliveryAddress.street}, {selectedRequest.deliveryAddress.city}, 
                    {selectedRequest.deliveryAddress.state} - {selectedRequest.deliveryAddress.pincode}
                    {selectedRequest.deliveryAddress.landmark && ` (${selectedRequest.deliveryAddress.landmark})`}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className={styles.progressSteps}>
                {selectedRequest.statusHistory.map((history, index) => {
                  const statusInfo = {
                    ...
                    confirmed: { icon: 'pi-check-circle', title: 'Order Confirmed' },
                    processing: { icon: 'pi-cog', title: 'Order Processing' },
                    ready_for_delivery: { icon: 'pi-box', title: 'Ready for Delivery' },
                    out_for_delivery: { icon: 'pi-truck', title: 'Out for Delivery' },
                    delivered: { icon: 'pi-check', title: 'Delivered' },
                    cancelled: { icon: 'pi-times-circle', title: 'Cancelled' }
                  }[history.status] || { icon: 'pi-info-circle', title: history.status };
                  ...
                  return (
                    <div key={index} className={styles.progressStep}>
                      ...
                      <div className={styles.stepContent}>
                        <div className={styles.stepTitle}>{statusInfo.title}</div>
                        <div className={styles.stepDescription}>{history.note || 'Status updated'}</div>
                        <div className={styles.stepDate}>{formatDate(history.timestamp)}</div>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className={styles.noDataMessage}>
            <i className="pi pi-exclamation-circle" style={{ fontSize: '2rem', color: 'var(--text-color-secondary)' }}></i>
  ...
  return (
    <div className={styles.medicineRequests}>
      ...
      {/* Enlarge Dialog */}
      <Dialog
        header={enlargedFile ? (enlargedFile.fileName || 'Prescription Preview') : 'Prescription Preview'}
