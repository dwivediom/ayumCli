...
export const loginInitate = async(data) => {
    try { 
      const logindata = await axios.post(loginUrl , {email:data.email ,  password:` key${data.sub}`} )
      return logindata
    } catch (error) {
...
export const registerinitate= async(data)=>{ 
     try{ 
      const registerData = await axios.post(registerUrl,{...data, password:` key${data.sub}`} )
      return registerData;
     }catch(error){ 
      console.log(error.message)
      return {error:error.message}
